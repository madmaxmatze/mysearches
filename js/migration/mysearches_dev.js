 ...
 */	
function MMMSearch_Search(_id, _section) {
	var id = _id;
 ...
 */	
function MMMSearch_Section(_id) {
	var id = _id;
	this.getId = function () {return id;};
	...
	this.addSearch = function(_searchId) {						// add a search to section
		searches[_searchId] = new MMMSearch_Search(_searchId, id);
...
function MMMSearch_Tab(_id) {
	...
	this.addSection = function (_id) {					// add a section to a tab in the setup
		sections[_id] = new MMMSearch_Section(_id);
		return sections[_id];
	};
}
...
function MMMSearch() {
	...
	this.getEnabledSearches = function () {		// returns a string with all active searches, seperated by a semicolon (needed to save which searches are active)
		...
		for (var search in searches) {
			if (typeof searches[search] == "object") {
				if (searches[search].isEnabled()) {
					enabledSearches[search] = searches[search];
				}
			}
		}
...
function MMMSearch_HTMLBuilder(_MMMSearch) {
	...
	/**
	 * easy access functions to DOM-methods - just for better source code style
	 */
	this.newText = function (_text) {
		return document.createTextNode(_text);
	};
	this.newObject = function (_kind, _id, _class) {
		var tmpNode = document.createElement(_kind);
		if (_id) {_this.setAttribute (tmpNode, "id", _id);}
		if (_class) {_this.setAttribute (tmpNode, "class", _class);}
		return tmpNode;
	};
	this.newDiv = function (_id, _class) {
		return this.newObject("div", _id, _class);
	};
	this.setAttribute = function (_obj, _attributeName, _attributeValue) {
    	if (_obj && typeof _obj == "object") {
	        var tmpAttribute = document.createAttribute(_attributeName);
			tmpAttribute.nodeValue = _attributeValue;
			_obj.setAttributeNode(tmpAttribute);
		}
	};
	
	var objects = [];
	this.setEvent = function (_obj, _event, _function) {
		if (_obj && typeof _function == "function") {
			_obj[_event] = _function;
			objects[_obj.id] = _obj;
		}
	};
	this.appendChild = function (_parent, _child) {
		if (_parent && _child) {
			_parent.appendChild(_child);
		}
		return _child;
	};
	this.insertBefore = function (_newObject, _insertBeforeObject) {
		if (_newObject && _insertBeforeObject) {
			_insertBeforeObject.parentNode.insertBefore(_newObject, _insertBeforeObject);
		}
	};
	this.replaceObject = function (_newObject, _oldObject) {
		if (_newObject && _oldObject) {
			this.removeAllChildren(_oldObject);
			this.removeEvents(_oldObject);
			_oldObject.parentNode.replaceChild(_newObject, _oldObject);
		}
	};
	this.removeAllChildren = function (_obj) {
		if (_obj) {
			while (_obj.firstChild) {
				this.removeObject(_obj.firstChild);
			}
		}
	};
	this.removeEvents = function (_obj) {
		// should be replaced by a framework which takes care of memory leaks or at least a hashmap which stores created events
		if (_obj) {
			if (_obj.onclick) {_obj.onclick = null;}
			if (_obj.onmouseover) {_obj.onmouseover = null;}
			if (_obj.onmouseout) {_obj.onmouseout = null;}
			if (_obj.onmousemove) {_obj.onmousemove = null;}
		}
	};
	this.removeObject = function (_obj) {
		if (_obj) {
			this.removeEvents(_obj);
			this.removeAllChildren(_obj);
			_obj.parentNode.removeChild(_obj);
		}
	};
	var objArray = [];
	this.setHtmlObj = function(_key, _value) {objArray[_key] = _value;};
	this.getHtmlObj = function(_key) {return document.getElementById(objArray[_key]);};
	this.getHtmlId = function(_key) {return objArray[_key];};
	this.createHtmlObj = function(_key, _type) {
		if (!_type) {_type = "div";}
		return this.newObject(_type, this.getHtmlId(_key), this.getHtmlId(_key));
	};
	...
	this.showModule = function() {
		...
		this.setHtmlObj("googleheader",		"nhdrwrap");
		this.setHtmlObj("googlefooter",		"footerwrap");
		
		this.setHtmlObj("main",				"MMMSearch_container");
		this.setHtmlObj("setupcontainer",	"MMMSearch_setup_container");
			
		this.setHtmlObj("content",			"MMMSearch_form");
		
		this.setHtmlObj("hint",				"MMMSearch_hint");
		this.setHtmlObj("input",			"MMMSearch_textinput");
		this.setHtmlObj("nosearchlabel",	"no_search_enabled");
		this.setHtmlObj("header",			"gsea");
		this.setHtmlObj("footer",			"footerwrap");
		
		this.setHtmlObj("inputcontainer",	"MMMSearch_input_container");


		this.setHtmlObj("inputcenter",		"MMMSearch_input_center");
		this.setHtmlObj("inputbgright",		"MMMSearch_input_right");

		this.setHtmlObj("searchicons",		"MMMSearch_searchicons");
...
/*
	this.getDomOfCSS = function() {
		var link = this.newObject("link");
		link.setAttribute("type", "text/css");
		link.setAttribute("rel", "stylesheet");
		link.setAttribute("href", MMMSearch.getPrefs("path") + "css/mysearches.css");
		return link;
	};
*/
	
	this.getDomOfSetupButton = function() {
		var imageUrl = MMMSearch.getPrefs("path") + "img/icons.png";
		var linkImage = _IG_GetCachedUrl(imageUrl);
		var icon = iconsMatching["mmmsearch_settings"];
		
		var div = this.createHtmlObj("setupbutton");
		div.style.backgroundImage="url" + "(" + linkImage + ")";
		div.style.backgroundPosition="-" + icon['x'] + "px -" + icon['y'] + "px";
		this.setEvent(div, "onclick", function () {_this.changeDisplaySetup();});
...
/*		
	this.getDomOfHideGoogleButton = function() {
		var div = this.createHtmlObj("hidegooglebutton");
		div.style.backgroundImage="url" + "('" + MMMSearch.getPrefs("path") + "img/mmmsearch_display_" + (MMMSearch.getPrefs('display_google') == "true" ? "block" : "none") + ".png')";
		this.setEvent(div, "onclick", function () {_this.changeDisplayGoogle();});
		this.setEvent(div, "onmouseover", function () {_this.changeHideGoogleButton(true);});
		this.setEvent(div, "onmouseout", function () {_this.changeHideGoogleButton(false);});
		this.setEvent(div, "onmousemove", function (e) {_this.onmousemove (e);});	
...
*/
	...
	this.changeSetupButton = function (_active){
		if (_active) {
			_this.onmouseover(MMMSearch.getTerm("button_setup"));
		} else {
			_this.onmouseout();
		}
		
		var icon = iconsMatching["mmmsearch_settings" + (_active ? "_active" : "")];
		_this.getHtmlObj("setupbutton").style.backgroundPosition="-" + icon['x'] + "px -" + icon['y'] + "px";
	};
	
	this.changeHomeButton = function (_active){
		if (_active) {
			_this.onmouseover(MMMSearch.getTerm("button_home"));
		} else {
			_this.onmouseout();
		}
		_this.getHtmlObj("homebutton").style.backgroundImage="url" + "(" + MMMSearch.getPrefs("path") + "img/mmmsearch_home" + (_active ? "_active" : "") + ".gif)";
	};
	
  this.changeMailButton = function (_active){
    if (_active) {
      _this.onmouseover(MMMSearch.getTerm("button_homepagelink_text"));
    } else {
      _this.onmouseout();
    }
    _this.getHtmlObj("mailbutton").style.backgroundImage="url" + "(" + MMMSearch.getPrefs("path") + "img/mmmsearch_mail" + (_active ? "_active" : "") + ".gif)";
  };
  
/*
	this.changeHideGoogleButton = function (_active){
		if (_active) {
			_this.onmouseover(_MMMSearch.getTerm("button_hidegoogle"));
		} else {
			_this.onmouseout();
		}

		_this.getHtmlObj("hidegooglebutton").style.backgroundImage="url('" + MMMSearch.getPrefs("path") + "img/mmmsearch_display_" + (MMMSearch.getPrefs('display_google') == "true" ? "block" : "none") + (_active ? "_hover" : "") + ".png')";
	}
*/
